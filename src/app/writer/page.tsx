...
type TabType =
  "overview" | "published" | "drafts" | "pending" | "rejected" | "trash";
...
export default function WriterDashboard() {
    ...
    const initialSeed: ArticleData[] = [
      {
        id: 1,
        title:
          "Federal Reserve Signals Shift in Upcoming Interest Rate Meeting",
        deck: "Policymakers weigh new labor market indicators before committing to quantitative adjustments in Washington.",
        content:
          "<p>Federal Reserve officials are preparing for what could be their most critical policy meeting of the fiscal year...</p>",
        category: "Economy",
        subcategories: [
        ...
        status: "draft",
        card_summary:
          "Federal Reserve officials analyze labor market data before their policy meeting.",
        focus_keyword: "Federal Reserve Interest Rates",
        meta_description:
          "Discover how the latest Federal Reserve meeting is set to shape policy and interest rate trends across national markets.",
  ...
  const handleDeleteArticle = async (article: ArticleData) => {
    ...
    try {
      await fetch(
        `/api/articles?id=${article.id}&email=${encodeURIComponent(user?.email || "")}&action=trash`,
        {
          method: "DELETE",
        },
      );
  ...
  const handlePermanentDelete = async (article: ArticleData) => {
    ...
    try {
      await fetch(
        `/api/articles?id=${article.id}&email=${encodeURIComponent(user?.email || "")}&action=permanent`,
        {
          method: "DELETE",
        },
      );
    ...
    >
      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm">
          {/* Table Header */}
          <div className="grid grid-cols-12 gap-4 px-6 py-3 border-b border-gray-100 bg-gray-50 text-[10px] font-bold text-gray-500 uppercase tracking-wider">
            <div className="col-span-12 md:col-span-6 lg:col-span-5">
              POST TITLE & SUMMARY
            </div>
            <div className="hidden lg:block col-span-2">CATEGORY</div>
            <div className="hidden md:block col-span-3 lg:col-span-2">DATE</div>
          ...
          <div className="divide-y divide-gray-100">
            {displayedArticles.map((article) => (
              <WriterPostCard
                ...
                onRestore={
                  activeTab === "trash" ? handleRestoreArticle : undefined
                }
                onPermanentDelete={
