      ...
      if (!res.ok) {
        ...
        setTimeout(() => {
          if (data.user.role === "admin") {
            ...
            window.location.href = "/"; // Reader goes to homepage
          }
        }, 1000);
      }
    } catch (err) {
      ...
      <div className="w-full max-w-[1440px] mx-auto relative z-10 flex h-screen">
        ...
        <div className="w-full md:w-[500px] lg:w-[560px] h-full bg-[#f4f4f4] flex flex-col items-center py-6 px-8 lg:px-14 overflow-y-auto shadow-2xl [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] relative">
          ...
          <div className="w-full bg-white p-6 sm:p-8 shadow-sm flex-col rounded-sm">
            ...
            {success && (
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full border border-gray-300 rounded px-3 py-2.5 text-[14px] text-gray-900 focus:outline-none focus:border-gray-500 bg-white"
                  required
                >
            ...
            <button
              type="button"
              className="w-full border border-black text-black bg-white rounded-full py-3 font-bold text-[14px] hover:bg-gray-50 transition-colors duration-200"
            >
